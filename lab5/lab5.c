 ...
 *
 *after newly freed block created check the if the next block is in the freelist
 and if it is we coalesce (newly_freed_block + nextblock)
 *
 *we check if the prev block is in the freelist and if it is we coalesce
 *
 *if there is no coalsence block we insert the new_coalsence_block in the free
 list
 *
 *after insertion, check if the free list remains sorted in the memory address
 order.
 *
 */
